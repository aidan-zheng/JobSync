...
export async function POST(request: NextRequest) {
  try {
      ...
      for (const { batch, relevanceResults } of chunkResults) {
        for (const relevance of relevanceResults) {
          ...
          const matchedApp = relevance.matched_application_id
            ? applications.find(
                (a) => a.application_id === relevance.matched_application_id,
              )
            : null;
    ...
    console.error("[scan-emails] unexpected failure:", err);
    return NextResponse.json(
      { error: "Unexpected server error while scanning emails." },
      { status: 500 },
    );
  }
}
