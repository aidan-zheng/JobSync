...
export async function POST(request: NextRequest) {
  try {
    ...
    const appDataList = new Map(
      apps?.map((a) => [
        a.application_id,
        { status: a.status, contact_person: a.contact_person },
      ]) ?? [],
    );
    ...
    for (
      let i = 0;
      i < deduplicatedSelectedEmails.length;
      i += CONCURRENCY_LIMIT
    ) {
      ...
      await Promise.all(
        chunk.map(async (selected) => {
          try {
            ...
            const appData = appDataList.get(selected.application_id) || {
              status: "applied",
              contact_person: null,
            };
            ...
            console.log(
              `[Process] Parsed output for msg ${selected.messageId}:`,
              parsed,
            );
            ...
            try {
              ...
              console.warn(
                `[Process] Invalid date from Gmail: ${message.date}. Using current time.`,
              );
            ...
            if (existingEmail) {
              ...
              if (emailErr || !emailRow) {
                console.error(
                  `[Process] Failed to insert email into DB:`,
                  emailErr,
                );
                return;
              }
    ...
    console.error("[scan-emails/process] unexpected failure:", err);
    return NextResponse.json(
      { error: "Unexpected server error while processing emails." },
      { status: 500 },
    );
  }
}
